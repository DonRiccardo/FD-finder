...
import cz.cuni.mff.fdfinder.depminerservice.algorithm.DepMinerSpark;
import cz.cuni.mff.fdfinder.depminerservice.algorithm.model._FunctionalDependency;
import cz.cuni.mff.fdfinder.depminerservice.model.DatasetDto;
import cz.cuni.mff.fdfinder.depminerservice.model.JobDto;
import cz.cuni.mff.fdfinder.depminerservice.model.JobResult;
import cz.cuni.mff.fdfinder.depminerservice.model.JobStatus;
...
public class DepMinerService {
    ...
    public DepMinerService(DiscoveryClient discoveryClient, RestClient.Builder restClientBuilder) {

        this.discoveryClient = discoveryClient;
        this.restClient = restClientBuilder.build();

        spark = SparkSession.builder()
                .appName("DEP-MINER-spark")
                .master("local")
                .getOrCreate();
    ...
    public void registerNewJob(JobDto job) {
        ...
        System.out.println("DEP-MINER - QUEUE: " + this.queue.size());
        if (!running.get()) {
            startNext();
        }
    }
    ...
    public void cancelJob(long jobId) {
        ...
        if (this.currentJob != null && this.currentJob.getId() == jobId) {
            ...
            System.out.println("DEP-MINER - CANCEL Job: " + this.currentJob.getId());
            System.out.println("DEP-MINER - CANCEL queue size: " + this.queue.size());
        }
        else {
    ...
    private void startNext() {
        ...
        System.out.println("DEP-MINER - ID poll: " + job.getId());

        running.set(true);
        currentJob = job;
        currentJobFuture = executor.submit(() -> {
            try {
                System.out.println("DEP-MINER - STARTING JOB: " + job.getId());
                this.actualSparkGroupId = getSparkContextGroupId(job.getId());
                sparkContext.setJobGroup(this.actualSparkGroupId, "DEP-MINER spark new job", true);
                runJob(currentJob);
            }
            catch (Throwable t) {
    ...
    private void runJob(JobDto job) {
        ...
        try {
            ...
            for (JobResult jobResult : job.getJobResults()) {
                ...
                System.out.println("DEP-MINER - JOB running: " + job.getId());

                DepMinerSpark algorithm = new DepMinerSpark(sparkContext, spark, datasetData.targetPathDataset,
                        datasetData.dataset.getName(), job.getSkipEntries(), job.getLimitEntries(),
                        job.getMaxLHS(), datasetData.dataset.getFileFormat(),
                        datasetData.dataset.getHeader(), datasetData.dataset.getDelim());

                jobResult.setStartTime(System.currentTimeMillis());
                System.out.println("DEP-MINER - JOB started at TIME: " + jobResult.getStartTime());
                ...
                System.out.println("DEP-MINER - JOB Spark finished at TIME: " + jobResult.getEndTime());
    ...
    private DatasetData getMetadataAndDownloadDataset(JobDto job) throws IOException {
        ...
        System.out.println("DEP-MINER - JOB retrieved = dataset ");
        ...
        System.out.println("DEP-MINER - JOB retrieved = file ");
    ...
    protected void processOneJobResult(JobResultsFDs jobResultsFDs, long jobId) {
        ...
        System.out.println("DEP-MINER Processing result of ONE JOB");
        ...
        try{
            ...
            System.out.println("DEP-MINER Processing finished successfully");
        }
        catch (FileAlreadyExistsException e){
            System.out.println("DEP-MINER Processing already exists");
        }
        catch (IOException e){
            System.out.println("DEP-MINER Processing failed" + e.getMessage());
        }
        finally {
    ...
    private void processResults(List<JobResultsFDs> jobResultsFDs, long jobId) {
        ...
        System.out.println("DEP-MINER Processing result of job");
    ...
    private void sendResultsToJobService(JobResult  jobResult, Path foundFdsFilePath) {
        ...
        if(!Files.exists(foundFdsFilePath)) {
            System.out.println("DEP-MINER - ERROR: " + foundFdsFilePath + " does not exist");
            return;
        }
        ...
        try {
            ...
            System.out.println("DEP-MINER - JOB SEND SUCCESSFULLY: " + response);

            Files.deleteIfExists(foundFdsFilePath);
        }
        catch (IOException e){
            System.out.println("DEP-MINER - JOB SENDING ERROR: " + e.getMessage());
        }
    ...
    private Path getFdsResultFilePath(JobResult jobResult, long jobId) {
        // fileName: job-ID-DEP-MINER-run-#-foundFDs.txt
        return Paths.get("tmp/results/job-" + jobId
                + "-DEP-MINER-run-" + jobResult.getIteration() + "-foundFDs.txt");
    }
