package cz.cuni.mff.fdfinder.distlatservice.service;

import cz.cuni.mff.fdfinder.distlatservice.algorithm.DistLatSpark;
import de.metanome.algorithms.depminer.depminer_helper.modules.container._FunctionalDependency;
import cz.cuni.mff.fdfinder.distlatservice.model.DatasetDto;
import cz.cuni.mff.fdfinder.distlatservice.model.JobDto;
import cz.cuni.mff.fdfinder.distlatservice.model.JobResult;
import cz.cuni.mff.fdfinder.distlatservice.model.JobStatus;
...
public class DistLatService {
    ...
    public DistLatService(DiscoveryClient discoveryClient, RestClient.Builder restClientBuilder) {

        this.discoveryClient = discoveryClient;
        this.restClient = restClientBuilder.build();

        spark = SparkSession.builder()
                .appName("DISTLAT-spark")
                .master("local")
                .getOrCreate();
    ...
    public void registerNewJob(JobDto job) {
        ...
        System.out.println("DISTLAT - QUEUE: " + this.queue.size());
        if (!running.get()) {
            startNext();
        }
    }
    ...
    public void cancelJob(long jobId) {
        ...
        if (this.currentJob != null && this.currentJob.getId() == jobId) {
            ...
            System.out.println("DISTLAT - CANCEL Job: " + this.currentJob.getId());
            System.out.println("DISTLAT - CANCEL queue size: " + this.queue.size());
        }
        else {
            for (JobDto job : this.queue) {

                if (job.getId() == jobId) {
                    System.out.println("DISTLAT - CANCEL Job = removed from queue: " + job.getId());
    ...
    private void startNext() {
        ...
        System.out.println("DISTLAT - ID poll: " + job.getId());
        ...
        currentJobFuture = executor.submit(() -> {
            try {
                ...
                System.out.println("DISTLAT - ERROR: " + t.getMessage());
                //t.printStackTrace();
            }
            finally {
                System.out.println("DISTLAT - FINISHED JOB: " + job.getId());
    ...
    private void runJob(JobDto job) {
        ...
        try {
            ...
            for (JobResult jobResult : job.getJobResults()) {
                ...
                System.out.println("DISTLAT - JOB running: " + job.getId());

                // initialize algorithm
                DistLatSpark algorithm = new DistLatSpark(sparkContext, spark, datasetData.targetPathDataset,
                ...
                System.out.println("DISTLAT - JOB started at TIME: " + jobResult.getStartTime());
                monitorService.startMonitoring(jobResult.getId());

                // get found FDs
                HashSet<_FunctionalDependency> foundFds = algorithm.startAlgorithm();
                ...
                System.out.println("DISTLAT - JOB Spark finished at TIME: " + jobResult.getEndTime());
            ...
            if (cancelled.get() && e instanceof SparkException) {
                ...
                System.out.println("DISTLAT - cancelled Spark Job: " + e.getMessage());
    ...
    private DatasetData getMetadataAndDownloadDataset(JobDto job) throws IOException {
        ...
        System.out.println("DISTLAT - JOB retrieved = dataset ");
        ...
        System.out.println("DISTLAT - JOB retrieved = file ");
     ...
     */
    @Async
    protected void processOneJobResult(JobResult jobResult, HashSet<_FunctionalDependency> fds, long jobId) {

        System.out.println("DISTLAT Processing result of ONE JOB");
        if (jobResult == null) {
        ...
        try{
            ...
            System.out.println("DISTLAT Processing finished successfully");
        }
        catch (FileAlreadyExistsException e){
            System.out.println("DISTLAT Processing already exists");
        }
        catch (IOException e){
            System.out.println("DISTLAT Processing failed" + e.getMessage());
        }
    }
     ...
     */
    private Path saveFoundFdsToResultFile(JobResult jobResult, HashSet<_FunctionalDependency> fds, long jobId) throws IOException {
    ...
    private void sendResultsToJobService(JobResult  jobResult, Path foundFdsFilePath) {
        ...
        if(!Files.exists(foundFdsFilePath)) {
            System.out.println("DISTLAT - ERROR: " + foundFdsFilePath + " does not exist");
            return;
        }
        ...
        try {
            ...
            System.out.println("DISTLAT - JOB SEND SUCCESSFULLY: " + response);

            Files.deleteIfExists(foundFdsFilePath);
        }
        catch (IOException e){
            System.out.println("DISTLAT - JOB SENDING ERROR: " + e.getMessage());
        }
    ...
    private Path getFdsResultFilePath(JobResult jobResult, long jobId) {
        ...
        return Paths.get("datatmp/results/job-" + jobId
                + "-DISTLAT-run-" + jobResult.getIteration() + "-foundFDs.txt");
    }
