...
import cz.cuni.mff.fdfinder.taneservice.model.JobStatus;
...
public class TaneService {
    ...
    public TaneService(DiscoveryClient discoveryClient, RestClient.Builder restClientBuilder) {
    ...
    public void registerNewJob(JobDto job) {
        ...
        System.out.println("TANE - QUEUE: " + this.queue.size());
        if (!running.get()) {
            startNext();
        }
    }
    ...
    public void cancelJob(long jobId) {
        ...
        if (this.currentJob != null && this.currentJob.getId() == jobId) {
            ...
            System.out.println("TANE - CANCEL Job: " + this.currentJob.getId());
            System.out.println("TANE - CANCEL queue size: " + this.queue.size());
        }
        else {
            for (JobDto job : this.queue) {

                if (job.getId() == jobId) {
                    System.out.println("TANE - CANCEL Job = removed from queue: " + job.getId());
    ...
    private void startNext() {
        ...
        currentJobFuture = executor.submit(() -> {
            try {
                System.out.println("TANE - STARTING JOB: " + job.getId());
                ...
                System.out.println("TANE - ERROR: " + t.getMessage());
                //t.printStackTrace();
            }
            finally {
                System.out.println("TANE - FINISHED JOB: " + job.getId());
    ...
    private void runJob(JobDto job) {
        ...
        try {
            ...
            for (JobResult jobResult : job.getJobResults()) {
                ...
                System.out.println("TANE - JOB running: " + job.getId());

                // initialize algorithm
                TaneSpark algorithm = new TaneSpark(sparkContext, spark, datasetData.targetPathDataset,
                ...
                System.out.println("TANE - JOB started at TIME: " + jobResult.getStartTime());
                ...
                System.out.println("TANE - JOB Spark finished at TIME: " + jobResult.getEndTime());
    ...
    private DatasetData getMetadataAndDownloadDataset(JobDto job) throws IOException {
        ...
        System.out.println("TANE - JOB retrieved = dataset ");
    ...
    protected void processOneJobResult(JobResult jobResult, HashSet<_FunctionalDependency> fds, long jobId) {
        ...
        System.out.println("TANE Processing result of ONE JOB");
        if (jobResult == null) {
        ...
        try{
            ...
            System.out.println("TANE Processing finished successfully");
        }
        catch (FileAlreadyExistsException e){
            System.out.println("TANE Processing already exists");
        }
        catch (IOException e){
            System.out.println("TANE Processing failed" + e.getMessage());
        }
    }
    ...
    private void sendResultsToJobService(JobResult  jobResult, Path foundFdsFilePath) {
        ...
        if(!Files.exists(foundFdsFilePath)) {
            System.out.println("TANE - ERROR: " + foundFdsFilePath + " does not exist");
            return;
        }
        ...
        try {
            ...
            System.out.println("TANE - JOB SEND SUCCESSFULLY: " + response);

            Files.deleteIfExists(foundFdsFilePath);
        }
        catch (IOException e){
            System.out.println("TANE - JOB SENDING ERROR: " + e.getMessage());
        }
