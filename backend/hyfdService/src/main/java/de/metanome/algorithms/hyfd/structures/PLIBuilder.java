...
/**
 * Builds Position List Indices (PLIs) or stripped partitions for relational data.
 */
public class PLIBuilder {
	...
	/**
	 * Returns the number of records processed in the last PLI calculation.
	 *
	...
	/**
	 * Constructs a PLIBuilder with a limit on the number of rows to process.
	 *
	 * @param inputRowLimit maximum number of rows to process; values ≤ 0 process all rows
	 */
	...
	/**
	 * Generates PLIs for all attributes of a relational input.
	 *
	 * @param relationalInput the input data
	 * @param numAttributes number of attributes in the input
	 * @param isNullEqualNull whether null values are considered equal
	 * @return a list of PositionListIndex, one per attribute
	 */
	public List<PositionListIndex> getPLIs(_Input relationalInput, int numAttributes, boolean isNullEqualNull) /*throws InputIterationException*/ {
	...
	/**
	 * Creates cluster maps for each attribute, mapping each unique value to the list of record indices.
	 *
	 * @param relationalInput the input data
	 * @param numAttributes number of attributes
	 * @return a list of maps, each mapping values to clusters of record indices
	 */
    protected List<HashMap<String, IntArrayList>> calculateClusterMaps(_Input relationalInput, int numAttributes) /*throws InputIterationException */{
            List<HashMap<String, IntArrayList>> clusterMaps = new ArrayList<>();
            for (int i = 0; i < numAttributes; i++)
                    clusterMaps.add(new HashMap<String, IntArrayList>());

            List<Tuple2<Row, Long>> data = relationalInput.getDataAsListAndIndex();

            this.numRecords = 0;

            for (Tuple2<Row, Long> entry : data){
                
                if(this.inputRowLimit == this.numRecords) break;
	...
	/**
	 * Converts cluster maps into PositionListIndex objects for each attribute.
	 * Clusters of size 1 are discarded.
	 *
	 * @param clusterMaps cluster maps for each attribute
	 * @param isNullEqualNull whether null values should be treated as equal
	 * @return a list of PositionListIndex objects
	 */
	...
	/**
	 * Static variant of getPLIs for preloaded data.
	 *
	 * @param records records represented as lists of strings
	 * @param numAttributes number of attributes
	 * @param isNullEqualNull whether null values are considered equal
	 * @return a list of PositionListIndex objects
	 */
	...
	/**
	 * Static version of calculateClusterMaps.
	 *
	 * @param records records represented as lists of strings
	 * @param numAttributes number of attributes
	 * @return a list of maps mapping attribute values to clusters of record indices
	 */
	...
	/**
	 * Static version of fetchPositionListIndexes.
	 *
	 * @param clusterMaps cluster maps for each attribute
	 * @param isNullEqualNull whether null values are treated as equal
	 * @return a list of PositionListIndex objects
	 */
