...
/**
 * Represents a node in a Functional Dependency (FD) tree structure.
 */
public class _FDTreeElement implements Serializable{
    protected _FDTreeElement[] children;
    protected BitSet rhsAttributes;
    protected boolean[] isfd;
    protected int maxAttributeNumber;

    public _FDTreeElement(int maxAttributeNumber) {
        this.maxAttributeNumber = maxAttributeNumber;
        children = new _FDTreeElement[maxAttributeNumber];
     ...
     * @return
     */
    public _FDTreeElement getChild(int i) {

        return children[i];
    }

    /**
     * Adds a right-hand-side (RHS) attribute to this node.
     *
     * @param i the attribute index to be added as a RHS attribute (0-based index).
     */
...
//		}
    ...
    /**
     * Retrieves and deletes a generalization of the given functional dependency {@code lhs -> a}.
     * <p>
     * If a generalization is found (a dependency with fewer attributes on the left-hand side),
     * it will be removed from the tree, and the corresponding attributes will be added to
     * {@code specLhs}.
     * </p>
     *
     * @param lhs the left-hand-side attribute set of the functional dependency
     * @param a the dependent attribute (right-hand side)
     * @param currentAttr the last attribute already processed from the LHS
     * @param specLhs a {@link BitSet} where the generalization’s left-hand side is stored
     * @return {@code true} if a generalization was found and deleted, {@code false} otherwise
     */
    ...
    /**
     * Recursively filters the tree to keep only the most specific (i.e., non-generalized)
     * functional dependencies.
     * <p>
     * This method traverses the tree and adds functional dependencies that do not have
     * a specialization in the {@code filteredTree}. It ensures that only the most
     * detailed dependencies are preserved.
     * </p>
     *
     * @param filteredTree the {@link _FDTree} instance where filtered dependencies are stored
     * @param activePath the current left-hand-side attribute path in the traversal
     */
    public void filterSpecializations(_FDTree filteredTree, BitSet activePath) {
        int attr;
        for (attr = 1; attr <= maxAttributeNumber; attr++) {
            if (children[attr - 1] != null) {
                activePath.set(attr);
    ...
    /**
     * Recursively filters the tree to retain only the most general functional dependencies.
     * <p>
     * This method removes redundant specializations by checking whether a generalization
     * already exists in the {@code filteredTree}. If not, the dependency is added.
     * </p>
     *
     * @param filteredTree the {@link _FDTree} that will contain the most general dependencies
     * @param activePath the current left-hand-side attribute path in the traversal
     */
    public void filterGeneralizations(_FDTree filteredTree, BitSet activePath) {
        int attr;
        for (attr = 1; attr <= maxAttributeNumber; attr++) {
            if (isfd[attr - 1]) {
    ...
    /**
     * Prints all functional dependencies stored in this subtree to the standard output.
     * <p>
     * Each dependency is printed in the form {@code {X1, X2, ...} -> A}, where the left-hand side
     * attributes are enclosed in braces and the right-hand side attribute follows the arrow.
     * </p>
     *
     * @param activePath the {@link BitSet} representing the current LHS path during traversal
     */
