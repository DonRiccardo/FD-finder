...
import cz.cuni.mff.fdfinder.fastfdsservice.model.JobStatus;
...
public class FastfdsService {
    ...
    public FastfdsService(DiscoveryClient discoveryClient, RestClient.Builder restClientBuilder) {

        this.discoveryClient = discoveryClient;
        this.restClient = restClientBuilder.build();

        spark = SparkSession.builder()
                .appName("FastFDs-spark")
                .master("local")
                .getOrCreate();
    ...
    public void registerNewJob(JobDto job) {
        ...
        System.out.println("FastFDs - QUEUE: " + this.queue.size());
        if (!running.get()) {
            startNext();
        }
    }
    ...
    public void cancelJob(long jobId) {
        ...
        if (this.currentJob != null && this.currentJob.getId() == jobId) {
            ...
            System.out.println("FastFDs - CANCEL Job: " + this.currentJob.getId());
            System.out.println("FastFDs - CANCEL queue size: " + this.queue.size());
        }
        else {
            for (JobDto job : this.queue) {

                if (job.getId() == jobId) {
                    System.out.println("FastFDs - CANCEL Job = removed from queue: " + job.getId());
    ...
    private void startNext() {
        ...
        System.out.println("FastFDs - ID poll: " + job.getId());

        running.set(true);
        currentJob = job;
        currentJobFuture = executor.submit(() -> {
            try {
                System.out.println("FastFDs - STARTING JOB: " + job.getId());
                this.actualSparkGroupId = getSparkContextGroupId(job.getId());
                sparkContext.setJobGroup(this.actualSparkGroupId, "FastFDs spark new job", true);
                runJob(currentJob);
            }
            catch (Throwable t) {
                System.out.println("FastFDs - ERROR: " + t.getMessage());
                //t.printStackTrace();
            }
            finally {
                System.out.println("FastFDs - FINISHED JOB: " + job.getId());
    ...
    private DatasetData getMetadataAndDownloadDataset(JobDto job) throws IOException {
        ...
        System.out.println("FastFDs - JOB retrieved = dataset ");
        ...
        System.out.println("FastFDs - JOB retrieved = file ");
    ...
    private void sendResultsToJobService(JobResult  jobResult, Path foundFdsFilePath) {
        ...
        if(!Files.exists(foundFdsFilePath)) {
            System.out.println("FastFDs - ERROR: " + foundFdsFilePath + " does not exist");
            return;
        }
        ...
        try {
            ...
            System.out.println("FastFDs - JOB SEND SUCCESSFULLY: " + response);

            Files.deleteIfExists(foundFdsFilePath);
        }
        catch (IOException e){
            System.out.println("FastFDs - JOB SENDING ERROR: " + e.getMessage());
        }
